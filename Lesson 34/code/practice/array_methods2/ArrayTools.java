...
public class ArrayTools {
    ...
    public static <T> T findByPredicate (T[] arr, Predicate <T> predicate) { // метод возвращает тип Т, из массива типа Т[], отбор по предикату
        for (int i = 0; i < arr.length; i++) {
            if (predicate.test(arr[i])) { // test - встроенный в класс Predicate метод, возвращает boolean - тест прошёл или нет
                return arr[i];
            }
        }
        return null;
    ...
    // bubbleSort
    public static <T extends Comparable <T>> void bubbleSort(T[] arr) { // этот метод применим к типам (классам), в которых есть Comparable <T>
        for (int i = 0; i < arr.length; i++) {
            for (int j = 0; j < arr.length - 1 - i; j++) {
                if (arr[j].compareTo(arr[j + 1]) > 0) { // результат двух соседних элементов > 0
                    ...
                    arr[j + 1] = t; // j + 1 - элемент поставили t
                }
            }
        }
    }
